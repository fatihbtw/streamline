...
const styles = `
  .search-title { font-family: 'Space Mono', monospace; font-size: 26px; font-weight: 700; color: #e8e8f0; margin-bottom: 24px; }
  ...
  .search-input { width: 100%; padding: 12px 16px 12px 44px; background: #0f0f1a; border: 1px solid #1a1a2e; border-radius: 10px; color: #e8e8f0; font-size: 15px; font-family: 'DM Sans', sans-serif; outline: none; transition: border-color 0.15s; }
  ...
  .type-badge { font-size: 11px; padding: 2px 6px; border-radius: 4px; font-weight: 500; }
  ...
  .add-btn { margin-top: auto; padding: 8px; background: #1a1a2e; border: 1px solid #2a2a4e; border-radius: 8px; color: #6366f1; font-size: 13px; font-weight: 500; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; font-family: 'DM Sans', sans-serif; transition: background 0.15s; }
  ...
  const handleAdd = async () => {
    ...
    try {
      await api.post('/media', {
        ...
        overview: item.overview, rating: item.rating, quality_profile: quality,
      });
      setAdded(true);
    ...
    <div className="result-card">
      ...
      <div className="result-body">
        ...
        <div className="result-meta">
          ...
          {item.rating > 0 && <span className="result-rating"><Star size={11} fill="currentColor" />{item.rating.toFixed(1)}</span>}
          <span className={`type-badge ${item.type === 'movie' ? 'type-movie' : 'type-series'}`}>{item.type === 'movie' ? 'Movie' : 'TV Show'}</span>
        </div>
        ...
        <button className={`add-btn ${added ? 'added' : ''}`} onClick={handleAdd} disabled={added || loading}>
          {added ? <><Check size={14} />Added</> : loading ? 'Adding...' : <><Plus size={14} />Add to Library</>}
        </button>
      </div>
    </div>
  );
    ...
    <>
      ...
      <form className="search-bar" onSubmit={handleSearch}>
        <div className="search-input-wrap">
          ...
          <input className="search-input" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search movies or TV shows..." />
        </div>
      ...
      {!searched ? <div className="empty-state"><Search size={48} style={{margin:'0 auto 12px',opacity:0.3,display:'block'}} />Enter a title to search for movies and TV shows</div>
        : loading ? <div className="empty-state">Searching...</div>
        : results.length === 0 ? <div className="empty-state">No results for "{query}"</div>
        : <div className="results-grid">{results.map(item => <ResultCard key={`${item.type}-${item.tmdb_id}`} item={item} />)}</div>}
    </>
  );
}
