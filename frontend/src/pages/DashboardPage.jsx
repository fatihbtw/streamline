...
import { Film, Tv, Download, CheckCircle, Clock, AlertCircle } from 'lucide-react';
...
const styles = `
  ...
  .stat-card {
    background: #0f0f1a; border: 1px solid #1a1a2e; border-radius: 12px; padding: 20px;
  ...
  .media-card {
    background: #0f0f1a; border: 1px solid #1a1a2e; border-radius: 10px; overflow: hidden;
    text-decoration: none; display: block; transition: transform 0.15s, border-color 0.15s;
  }
  ...
  return (
    <>
      ...
      <div className="dash-header">
        ...
        <div className="dash-sub">Übersicht deiner Mediathek</div>
      </div>

      <div className="stats-grid">
        <StatCard icon={Film} value={movies.length} label="Filme" color="#6366f1" />
        <StatCard icon={Tv} value={series.length} label="Serien" color="#8b5cf6" />
        <StatCard icon={Download} value={downloading.length} label="Downloads aktiv" color="#f59e0b" />
        <StatCard icon={CheckCircle} value={downloaded.length} label="Abgeschlossen" color="#10b981" />
      </div>

      <div className="section-title">Zuletzt hinzugefügt</div>
      {loading ? (
        <div className="empty-state">Lade...</div>
      ) : data.items.length === 0 ? (
        <div className="empty-state">Noch keine Medien. <Link to="/search" style={{ color: '#6366f1' }}>Jetzt suchen →</Link></div>
      ) : (
          ...
          {data.items.slice(0, 16).map(item => (
            <Link key={item.id} to={`/library/${item.id}`} className="media-card">
              ...
              <div className="media-info">
                ...
                <div className="media-meta">{item.year} · {item.type === 'movie' ? 'Film' : 'Serie'}</div>
